...
test("posting one task", async () => {
  ...
  expect(getAfterPost.data.data.length).toBe(
    getBeforePost.data.data.length + 1
  );
});
...
test("postRequest", async () => {
  ...
  const response = await functions.postRequest({
    partnerID: "123456789123456789123456",
    description: "new3",
    consult: true
  });
...
test("getRequest", async () => {
  ...
  expect(response.data.data[response.data.data.length - 1].description).toBe(
    "new4"
  );
});
...
test("deleterequest", async () => {
  expect.assertions(1);
  const postReq = await functions.postRequest({
    partnerID: "123456789123456789123456",
    description: "new6",
    consult: true
  });
...
test("deleting one eventRequest", async () => {
  ...
  const id = getBefore.data.data.length - 1;
  const deleted = await functions.deleteOneEventRequest(
    getBefore.data.data[id]["_id"]
  );
...
test("Searching tasks by payment", async () => {
  ...
  const resultSet = await functions.searchTasksByPayment(money);
  let match = true;

  for (let i = 0; i < resultSet.data.data.length; i++) {
    if (
      !(
        resultSet.data.data[i]["payment"] >= money + 50 ||
        resultSet.data.data[i]["payment"] <= money + 50
      )
    )
      match = false;
...
test("posting a new event ", async () => {
  const params = {
    ...
    description:
      "event in hurghada kabf kjb kjabsfk bakdfb akbf kjabfkj habjfbh ajsfhb jabshf jhbasfj akjafsb kjb ",
    type: "for fun ",
  ...
  expect(newEvent.data.data.description).toEqual(
    "event in hurghada kabf kjb kjabsfk bakdfb akbf kjabfkj habjfbh ajsfhb jabshf jhbasfj akjafsb kjb "
  );
});
...
test("editing an event ", async () => {
  const params = {
    description:
      "this a meaningless description used for testing and just help me exceed the thirt charachters i wish to exceedddddddddddddddddddddddddddddddd "
  };
  const updatedEvent = await functions.updatingAnEvent(
    "5c9534d9245cba7ddab501dc",
    params
  );
  //console.log(updatedEvent.config.data)
  expect(updatedEvent.config.data).toEqual(
...
test("deleting an Event ", async () => {
  ...
  const deletedEvent = await functions.deleteAnEvent(
    allBefore.data.data[id]["_id"]
  );
...
test("gets all members", async () => {
  ...
  //  //console.log(response)
  expect(response.data.data[response.data.data.length - 1].email).toBe(
    "takeCareAgain55555@gmail.com"
  );
});
...
test("loop for all results to be sure that they are in that location", async () => {
  expect.assertions(1);
  let flag = true;
  const events = await functions.searchEventByLocation(
    "cairo",
    "youssef el sebaie",
    "rehab"
  );
...
test("deleting one eventBooking", async () => {
  ...
  const id = getBefore.data.data.length - 1;
  const deleted = await functions.deleteOneEventBooking(
    getBefore.data.data[id]["_id"]
  );
